class GeneExpressions
  ...
  attr_reader :percentiles_over_samples, :percentiles_over_genes

  # samples = [sample_1, sample_2,...]
  # gene_expressions = {hgnc_1 => [expression_for_sample_1, expression_for_sample_2,...], ...}
  # hgnc_id should be integer
  # percentiles_over_<...> = {hgnc_1 =>[percentile_for_sample_1, percentile_for_smaple_2,...], ...}
  ...
  def initialize(samples, gene_expressions, gene_symbols)
    ...
    @percentiles_over_samples = gene_expressions.map{|hgnc_id, expressions_over_samples|
      [hgnc_id.to_i, calculate_percentiles(expressions_over_samples)]
    }.to_h
    ...
    percentiles_over_genes_by_sample_id = samples.map{|sample|
      expressions_over_genes = expressions_by_sample(sample.sample_id).values_at(*gene_hgnc_ids)
      [sample.sample_id, calculate_percentiles(expressions_over_genes)]
    }.to_h

    @percentiles_over_genes = gene_hgnc_ids.map{|hgnc_id|
      gene_index = index_of_gene_hgnc_id[hgnc_id]
      sample_percentiles_for_gene = samples.map{|sample|
        percentiles_over_genes_by_sample_id[sample.sample_id][gene_index]
      }
      [hgnc_id.to_i, sample_percentiles_for_gene]
    }.to_h
  end
  ...
  def expressions_with_percentiles_by_hgnc(hgnc_id)
    hgnc_id = hgnc_id.to_i
    expressions = @gene_expressions[hgnc_id]
    expressions ? @samples.zip(
                    expressions,
                    percentiles_over_genes[hgnc_id],
                    percentiles_over_samples[hgnc_id]
                  ) : {}
  end
  ...
  def expressions_with_percentiles_by_sample(sample_id)
    sample_index = @index_of_sample_cache[sample_id]
    @gene_expressions.map{|hgnc_id, expressions_for_samples|
      [ hgnc_id,
        expressions_for_samples[sample_index],
        percentiles_over_genes[hgnc_id][sample_index],
        percentiles_over_samples[hgnc_id][sample_index]
      ]
    }
  end
  ...
  # percentile of element T is defined as fraction of elements which are strictly less than T
  def calculate_percentiles(arr)
    arr.each_with_index
      ...
      .sort_by{|percentile, ind| ind }
      .map{|percentile, ind| percentile }
  end
